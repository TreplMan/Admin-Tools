...
function imgui.BeforeDrawFrame()
    if fa_font == nil then
        ...
        if fontsize30 == nil and fontsize20 == nil and fontsize50 == nil then
            ...
            fontsize30 = imgui.GetIO().Fonts:AddFontFromFileTTF(getFolderPath(0x14) .. '\\trebucbd.ttf', 30.0, nil, imgui.GetIO().Fonts:GetGlyphRangesCyrillic()) -- âìåñòî 30 ëþáîé íóæíûé ðàçìåð
...
local script_url = "https://raw.githubusercontent.com/TreplMan/Admin-Tools/main/!ATools.lua" --ññûëêà íà îáíîâëåííûé ôàéë
...
function main()
    ...
    sampAddChatMessage('{FF0000}[AdminTools] {FF8C00}Ñêðèïò áûë èíèöèàëèçèðîâàí. (by Mason_Baker)', stColor)
	sampAddChatMessage('{FF0000}[AdminTools] {FF8C00}Àêòèâàöèÿ ñêðèïòà - /rh', stColor)
    --Ïðîâåðêà íàëè÷èÿ îáíîâëåíèÿ
    downloadUrlToFile(update_url, update_path, function(id, status)
        if status == dlstatus.STATUS_ENDDOWNLOADDATA then
            updateIni = inicfg.load(nil, update_path)
            if updateIni ~= nil then
                if updateIni.info.version > version_int then
                    sampAddChatMessage("{FF0000}[AdminTools] {FF8C00}Åñòü îáíîâëåíèå! Âåðñèÿ: "..updateIni.info.version, -1)
                    update = true
                elseif updateIni.info.version == version_int then
                    sampAddChatMessage("{FF0000}[AdminTools] {FF8C00}Ó âàñ ñòîèò àêòóàëüíàÿ âåðñèÿ: "..updateIni.info.version, -1)
                end
            end
    ...
    BindAlock = rkeys.registerHotKey(ofHotkeys.Alock.v, 1, false, function()
        if not sampIsChatInputActive() and not sampIsDialogActive() then
			if elements.checkbox.alock.v then
				...
				sampAddChatMessage('{FF0000}[AdminTools] {FF8C00}Êóïè êîìàíäó â /adm. Åñëè êóïèë, ïîñòàâü ÷åê-áîêñ', stColor)
			end
        end
    end)
...
function imgui.OnDrawFrame()
    if  AdminTools.v then
        ...
        if imgui.Button(fa.ICON_FA_DOWNLOAD, imgui.ImVec2(25, 25)) then
            if update then
                sampAddChatMessage('{FF0000}[AdminTools] {FF8C00}Îáíîâëÿþñü', stColor)
                start_update = true
            else
                sampAddChatMessage('{FF0000}[AdminTools] {FF8C00}Îáíîâëåíèé íåò.', stColor)
            end
        end imgui.SameLine()
        ...
        imgui.centeredText(u8"Ãëàâíîå ìåíþ")
        imgui.PopFont()
        if imgui.InvButton(fa.ICON_FA_USER..u8" Ïðîôèëü", imgui.ImVec2(150, 0)) then
            sampAddChatMessage('{FF0000}[AdminTools] {FF8C00}Êðàñàâà', stColor)
            menuSelect = 1
        end
        if imgui.InvButton(fa.ICON_FA_COG..u8" Íàñòðîéêè", imgui.ImVec2(150, 0)) then
            sampAddChatMessage('{FF0000}[AdminTools] {FF8C00}Êðàñàâà', stColor)
            menuSelect = 2
        end
        if imgui.InvButton(fa.ICON_FA_CHART_LINE..u8" Ñòàòèñòèêà", imgui.ImVec2(150, 0)) then
            sampAddChatMessage('{FF0000}[AdminTools] {FF8C00}Êðàñàâà', stColor)
            menuSelect = 3
        end
        if imgui.InvButton(fa.ICON_FA_CHECK_SQUARE..u8" Ôîðìû", imgui.ImVec2(150, 0)) then
            sampAddChatMessage('{FF0000}[AdminTools] {FF8C00}Êðàñàâà', stColor)
            menuSelect = 4
        end
        if imgui.InvButton(fa.ICON_FA_REPLY..u8" Îêíî Æàëîá", imgui.ImVec2(150, 0)) then
            sampAddChatMessage('{FF0000}[AdminTools] {FF8C00}Êðàñàâà', stColor)
            menuSelect = 5
        end
        if imgui.InvButton(fa.ICON_FA_PLANE..u8" Ìåíþ Òåëåïîðòîâ", imgui.ImVec2(150, 0)) then
            sampAddChatMessage('{FF0000}[AdminTools] {FF8C00}Êðàñàâà', stColor)
            menuSelect = 6
        end
        if imgui.InvButton(fa.ICON_FA_TH_LIST..u8" Òàáëèöà íàêàçàíèé", imgui.ImVec2(150, 0)) then
            sampAddChatMessage('{FF0000}[AdminTools] {FF8C00}Êðàñàâà', stColor)
            menuSelect = 7
        end
        if imgui.InvButton(fa.ICON_FA_LIST..u8" Àäìèí-Êîìàíäû", imgui.ImVec2(150, 0)) then
            sampAddChatMessage('{FF0000}[AdminTools] {FF8C00}Êðàñàâà', stColor)
            menuSelect = 8
        end
        if imgui.InvButton(fa.ICON_FA_PLUS..u8" Àäìèí-Ïëþøêè", imgui.ImVec2(150, 0)) then
            sampAddChatMessage('{FF0000}[AdminTools] {FF8C00}Êðàñàâà', stColor)
            menuSelect = 8
        end
        imgui.EndChild()
        ...
        if menuSelect == 1 then
            ...
            if HLcfg.admSetting.admNick == "" or HLcfg.admSetting.admlvl == 0 then
                ...
                imgui.Text(u8"Ââåäèòå âàø Íèê =>> ") imgui.SameLine() imgui.InputText('##nickadm', elements.input.admNick)
                imgui.PushItemWidth(50)
                imgui.Text(u8"Ââåäèòå âàø Óðîâåíü Àäìèíèñòðàòîðà =>> ") imgui.SameLine() imgui.InputText('##lvladm', elements.input.admlvl)
                ...
                imgui.Text(u8"Çäðàñòâóéòå "..HLcfg.admSetting.admNick..u8". Óðîâåíü àäìèíèñòðàòîðà - "..HLcfg.admSetting.admlvl)
                imgui.PopFont()
            end
            ...
            if imgui.Checkbox(u8"[Âêë/âûêë] Àâòî-âõîä êàê ADM", elements.checkbox.autoCome) then
                HLcfg.admSetting.autoCome = elements.checkbox.autoCome.v
                save()
            end imgui.SameLine() imgui.HelpMarker(u8"Íå íàäî ââîäèòü àäìèí-ïàðîëü ñàìîìó, ñêðèïò ñäåëàåò ýòî çà âàñ")
            if elements.checkbox.autoCome.v then
                imgui.Text(u8"Ââåäèòå àäìèí-ïàðîëü: ")  imgui.PushItemWidth(100)
                if imgui.InputText("##adminPassword", elements.input.adminPassword, (elements.checkbox.showAdminPassword.v and imgui.InputTextFlags.Password or nil)) then
                    ...
                    save()
                end imgui.PopItemWidth() imgui.SameLine() if imgui.ToggleButton('Àäìèí Ïàðîëü', elements.checkbox.showAdminPassword) then
                    HLcfg.admSetting.showAdminPassword = elements.checkbox.showAdminPassword.v
                    save()
                end imgui.SameLine() imgui.HelpMarker(u8"Íàñòðîéêà, êîòîðàÿ áóäåò ïîêàçûâàòü, îòîáðàçèòüñÿ âàø àäìèí-ïàðîëü, èëè íåò")
            end
            imgui.EndChild()
            imgui.SameLine()
            imgui.BeginChild('##pass', imgui.ImVec2(278, 100), true)
            if imgui.Checkbox(u8"[Âêë/âûêë] Àâòî-âõîä â àêêàóíò", elements.checkbox.autoCome2) then
                HLcfg.admSetting.autoCome2 = elements.checkbox.autoCome2.v
                save()
            end imgui.SameLine() imgui.HelpMarker(u8"Íå íàäî ââîäèòü ïàðîëü ñàìîìó, ñêðèïò ñäåëàåò ýòî çà âàñ")
            if elements.checkbox.autoCome2.v then
                imgui.Text(u8"Ââåäèòå ïàðîëü: ") imgui.PushItemWidth(100)
                if imgui.InputText("##Password", elements.input.Password, (elements.checkbox.showPassword.v and imgui.InputTextFlags.Password or nil)) then
                    ...
                    save()
                end imgui.PopItemWidth() imgui.SameLine() if imgui.ToggleButton('Ïàðîëü', elements.checkbox.showPassword) then
                    HLcfg.admSetting.showPassword = elements.checkbox.showPassword.v
                    save()
                end imgui.SameLine() imgui.HelpMarker(u8"Íàñòðîéêà, êîòîðàÿ áóäåò ïîêàçûâàòü, îòîáðàçèòüñÿ âàø ïàðîëü, èëè íåò")
            end
            imgui.EndChild()
        end
        if menuSelect == 2 then
            ...
            if imguiad.HotKey("##ActiveOne", ofHotkeys.LeaveReconWindow, tLastOne, 100) then
                ...
                luasave()
            end imgui.SameLine() imgui.Text(u8"Âûéòè èç ðåêîíà")
            local tLastThree = {}
            if imguiad.HotKey("##ActiveThree", ofHotkeys.openHomeWindow, tLastThree, 100) then
                ...
                luasave()
            end  imgui.SameLine() imgui.Text(u8'Îòêðûòü îñíîâíîå îêíî')
            local tLastFour = {}
            if imguiad.HotKey("##ActiveFour", ofHotkeys.openAutoReport, tLastFour, 100) then
                ...
                luasave()
            end imgui.SameLine() imgui.Text(u8'Îòêðûòü àâòî-ðåïîðò')
            local tLastFive = {}
            if imguiad.HotKey("##ActiveFive", ofHotkeys.enabledTracers, tLastFive, 100) then
                ...
                luasave()
            end imgui.SameLine() imgui.Text(u8'Âêë/âûêë òðåéñåðû')
            if imguiad.HotKey("##ActiveSix", ofHotkeys.Alock, tLastOne, 100) then
                ...
                luasave()
            end imgui.SameLine() imgui.Text(u8"Îòêðûòèå ëþáîãî àâòî")
            if imguiad.HotKey("##ActiveSeven", ofHotkeys.DelVeh, tLastOne, 100) then
                ...
                luasave()
            end imgui.SameLine() imgui.Text(u8"Óäàëåíèå Ò/Ñ")
            imgui.EndChild()
        end
        if menuSelect == 3 then
            imgui.PushFont(fontsize20)
            imgui.centeredText(u8"Ñòàòèñòèêà")
            imgui.PopFont()
            imgui.Text(u8'Íàêàçàíèÿ çà âñå âðåìÿ.') imgui.SameLine() imgui.SetCursorPosX(265) imgui.Text(u8'Ñòàòèñòèêà çà äåíü çà äåíü')
            imgui.BeginChild('##nakaz', imgui.ImVec2(240, 280), true)
                imgui.Text(u8'Êîëè÷åñòâî /ban: '..HLcfg.Count.ban)
                imgui.Text(u8'Êîëè÷åñòâî /iban: '..HLcfg.Count.iban)
                imgui.Text(u8'Êîëè÷åñòâî /warn: '..HLcfg.Count.warn)
                imgui.Text(u8'Êîëè÷åñòâî /offwarn: '..HLcfg.Count.offwarn)
                imgui.Text(u8'Êîëè÷åñòâî /mute: '..HLcfg.Count.mute)
                imgui.Text(u8'Êîëè÷åñòâî /rmute: '..HLcfg.Count.rmute)
                imgui.Text(u8'Êîëè÷åñòâî /prison: '..HLcfg.Count.prison)
                imgui.Text(u8'Êîëè÷åñòâî /kick: '..HLcfg.Count.kick)
                imgui.Separator()
                imgui.Text(u8'Êîëè÷åñòâî ïðèíÿòûõ ôîðìû: '..HLcfg.Count.forms)
                imgui.Text(u8'Êîëè÷åñòâî ïðèíÿòûõ æàëîá: '..HLcfg.Count.reports)
            imgui.EndChild()
            imgui.SameLine()
            imgui.BeginChild('##alltday', imgui.ImVec2(240, 200), true)
                imgui.centeredText(u8'Ôîðì çà äåíü: '..HLcfg.config.dayForms)
                imgui.centeredText(u8'Ôîðì çà ñåàíñ: '..LsessionForma)
                imgui.centeredText(u8'Ðåïîðòîâ çà äåíü: '..HLcfg.config.dayReports)
                imgui.centeredText(u8'Ðåïîðòîâ çà ñåàíñ: '..LsessionReport) 
                imgui.centeredText(u8'Îíëàéí çà ñåàíñ: '..get_clock(sessionOnline.v))
                imgui.centeredText(u8'Îíëàéí çà äåíü: '..get_clock(HLcfg.onDay.online)) 
                imgui.centeredText(u8'ÀÔÊ çà ñåàíñ: '..get_clock(sessionAfk.v)) 
                imgui.centeredText(u8'ÀÔÊ çà äåíü: '..get_clock(HLcfg.onDay.afk))
            imgui.EndChild()
            imgui.Text(u8'Îíëàéí çà âñå âðåìÿ.')
            imgui.BeginChild('##alltime', imgui.ImVec2(240, 100), true)
                imgui.Text(u8'Îíëàéí çà âñå âðåìÿ: '..get_clock(HLcfg.Count.Online))
                imgui.Text(u8'AFK çà âñå âðåìÿ: '..get_clock(HLcfg.Count.AFK))
            imgui.EndChild()
        end
        imgui.EndChild()
        imgui.End()
